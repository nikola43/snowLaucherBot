...
contract Token is ERC20Burnable, Ownable {
    ...
    constructor(
        ...
        router = IJoeRouter02(addresses[0]);
        dividentTokenBridgeAddress = router.WAVAX();

        // Create a uniswap pair for this new token
        lpPair = IJoeFactory(router.factory()).createPair(
            address(this),
            router.WAVAX()
        );
    ...
    function swapAndSendDividends(uint256 tokenAmount) private {
        bool isNativeToken = address(dividendTokenAddress) == router.WAVAX();

        if (isNativeToken) {
            swapTokensForNative(tokenAmount, address(this));
        } else {
            swapTokensForTokens(
    ...
    function _finalizeTransfer(
        ...
        if (takeFee) {
            ...
            uint256 feeAmount1 = calcBuySellTransferFee(from, to, amount);

            if (feeAmount1 > 0) {
                // we substract fee amount from recipient amount
                amountReceived = amount - feeAmount1;

                // and transfer fee to contract
                super._transfer(from, address(this), feeAmount1);
            }
        }
